          ...
          >
            {filteredItems.length > 0 ? (
              filteredItems.map((item) => (
                <div
                  key={item.code}
                  className={`relative flex cursor-pointer select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none hover:bg-accent hover:text-accent-foreground ${(value === item.code || value === item.name) ? "bg-accent" : ""
                    }`}
                  onClick={() => {
                    onChange(item.code)
                    setIsOpen(false)
  ...
  useEffect(() => {
    const fetchCourseAndSchedule = async () => {
      ...
      try {
        ...
        // ✅ Check registration form type and redirect if needed
        const formType = scheduleData.registration_form_type || 'default'
        if (formType === 'acls') {
          window.location.href = `/guest-training-registration-acls?schedule_id=${scheduleId}`
          return
        } else if (formType === 'bls') {
          window.location.href = `/guest-training-registration-bls?schedule_id=${scheduleId}`
          return
                ...
                <div className="flex items-center gap-2">
                  <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="text-right">
                      ...
                      <p className={`text-[10px] font-medium leading-tight ${scheduleEventType === 'online'
